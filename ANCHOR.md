...
- **待真机复测**：安卓对焦/解码率是否恢复 v1.4 水准、重复计数是否显著下降（#9×#10 修复见效）、
  iPhone 无回归、加载提示与耗时读数实际可用性；数据回来后定 #8 Worker 池。
...
- **#10 换帧门控 + #5b 帧同步信标 —— ✅ v1.7.0 落地、v1.7.1 修正（#5a 对焦锁定真机证伪回退；#9×#10 矩形冲突修复），待真机复测 v1.7.1**。
  复测看点：安卓对焦/解码率恢复度（对照 v1.4 的 ~10/s）、重复计数降幅（#9×#10 修复见效）、iPhone 无回归、
  加载提示与耗时读数可用性；以完成卡「耗时/KB/s」为跨版本比照口径。
